...
// Endpoint for generating a title
server.post('/generate-title', async (req, res) => {
    const { content } = req.body;
    console.log("Requesting title generation with content:", content);

    try {
        const response = await axios.post(`${PYTHON_SERVICE_URL}/generate-title`, { content });
        console.log("Generated title:", response.data.title);
        res.json(response.data);
    } catch (error) {
        console.error("Error in title generation:", error.message);
        res.status(500).json({ message: "Failed to generate title" });
    }
});


// Endpoint for generating a cover image
server.post('/generate-cover-image', async (req, res) => {
    const { title } = req.body;
    console.log("Forwarding cover image generation request with content title:", title);

    try {
        const response = await axios.post(`${PYTHON_SERVICE_URL}/generate-cover-image`, { title });
        const imageUrl = response.data.image_url;
        console.log("Generated cover image URL:", imageUrl);
        console.log("Downloading cover image from URL:", imageUrl);
        ...
        const fileName = `cover-image-${nanoid()}.jpeg`;

        // Upload the image to S3
        const s3Url = await uploadToS3(responseImage.data, fileName);
        console.log("Image uploaded to S3:", s3Url);
        
        // Send the S3 URL back to the client
        res.json({ image_url: s3Url });
    } 
    catch (error) {
        console.error('Error generating cover image:', error.message);
        res.status(500).send({ message:'Failed to generate cover image'});
    }
});
